...
$UniNum = $_POST["userPassword"]; // 아이템 고유번호를 구별해주는 역할
 $data = array(); //data라는 배열을 만듭니다. 
    ...
    if($gameName == "MapleStory"){  //MapleStory라는 게임이라면
        ...
        while($row= mysqli_fetch_array($q)){  //SQL문으로 출력한 만큼 1개씩 리턴해줍니다. 
            $data["success"]=true;//통신이 되었습니다.
            $data["ItemName"]=$row[0];//아이템 이름
            $data["ItemInfo"]=$row[1];//아이템 정보
        }
        ...
        echo json_encode($data); //아이템 이름, 정보를 출력합니다. 
        mysqli_close($con); // 연결을 끝어버립니다. 
