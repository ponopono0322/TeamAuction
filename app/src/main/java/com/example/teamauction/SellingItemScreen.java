...
public class SellingItemScreen extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //판매중인 아이템 리스트 정보를 DB에서 받아와 리스트뷰에 보여줌
            ...
            public void onResponse(String response) { // onResponse 함수 재정의
                try { // 만약 반응이 왔고 정상적이라면
                    ...
                    for (int i = 0; i < games.length(); i++) { // games의 길이만큼 반복문 수행
                        ...
                        String Uniumber = item.getString("UniNum"); // UniNum 데이터 저장
        ...
        String purl = "http://ualsgur98.dothome.co.kr/SInfo.php"; // 통신할 php 주소
        // 데이터 전송을 위한 데이터 세팅
        PHPRequest validateRequest = new PHPRequest( purl, myGameName,myCharName,responseListener);
        RequestQueue queue = Volley.newRequestQueue(SellingItemScreen.this);  // 큐를 생성
       ...
       Response.Listener<String> responseListener1 = new Response.Listener<String>() {
        ...
        RequestQueue queue1 = Volley.newRequestQueue(SellingItemScreen.this); // 큐를 생성
        ...
        dropingbutton.setOnClickListener(new View.OnClickListener() { // 구매하기 버튼 클릭 이벤트 생성
            ...
            public void onClick(View view) { // onClick 함수 재정의
                ...
                if (pos > -1) { // pos는 0이상이므로 그 이상일 때만 작동
                    ...
                    startActivity(intent); // DeleteScreen 실행
                    finish(); // 현재 엑티비티 종료
                }
            }
        });
        ...
        fixingbutton.setOnClickListener(new View.OnClickListener() { // 구매하기 버튼 클릭 이벤트 생성
            ...
            public void onClick(View view) { // onClick 함수 재정의
                ...
                if (pos > -1) { // pos는 0이상이므로 그 이상일 때만 작동
                    ...
                    startActivity(intent); // FixScreen 실행
                    finish(); // 현재 엑티비티 종료
                }
            }
        });
